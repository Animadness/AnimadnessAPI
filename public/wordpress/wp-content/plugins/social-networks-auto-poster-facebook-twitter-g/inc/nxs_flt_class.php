   ...
   <div class="nxs_tls_bdX">          
    ...
    public static function print_dates_section( $current_post, $metaSettings, $nt='', $ii='') { $isVis =  !empty($metaSettings['nxs_tags_names']) || !empty($metaSettings['nxs_cats_names']);
        ...
        for( $j = 1; $j <= $count_abs_periods; $j++ ) {  $postfix = $j == 1 ? '' : '_'. $j;  $rel = $j == 1 ? '' : 'nxs_date_abs_period_'. $j;
            ...
            if( $j > 1 ) echo '<hr>';            
            echo '<div class="'. 'nxs_small_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Postes newer than', 'social-networks-auto-poster-facebook-twitter-g' ) .':</label>';
            ...
            echo '<div class="'. 'nxs_small_field" rel="'. $rel .'">';
            echo '<label class="field_title">'. __( 'Postes older than', 'social-networks-auto-poster-facebook-twitter-g' ) .':</label>';
    ...
    public static function print_meta_section( $current_post, $metaSettings, $nt='', $ii='' ) { $isVis = !empty($metaSettings['nxs_meta_key']); $ntN = $nt.$ii;
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i;    $rel = $i == 1 ? '' : 'nxs_meta_compare_'. $i;      if ($i>1 && empty($metaSettings["nxs_meta_key$postfix"])) continue;   
          ...
          self::printSelect( "nxs".$nt.$ii."_meta_relation", self::makeInputName("nxs_meta_relation$postfix", $nt, $ii ), false, $relation_options, !empty($metaSettings["nxs_meta_relation$postfix"])?$metaSettings["nxs_meta_relation$postfix"]:'', !empty($current_post->ID)?$current_post->ID:0, false, 'notTknz');
        ...
        <div class="">
          <div class="">
            <div class="nxs_medium_field_txn"> <label class="field_title"> <?php _e( 'Custom Field Name', 'social-networks-auto-poster-facebook-twitter-g' ); ?>:</label><br/>
               <input name="<?php echo self::makeInputName("nxs_meta_key$postfix", $nt, $ii); ?>" id="nxs<?php echo $nt.$ii; ?>_meta_key<?php echo $postfix; ?>" style="font-weight: bold; color: #005800; border: 1px solid #ACACAC; width: 95%;" value="<?php echo !empty($metaSettings["nxs_meta_key$postfix"])?$metaSettings["nxs_meta_key$postfix"]:'';?>"/>
            </div>
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $compare_options, "nxs".$nt.$ii."_meta_operator$postfix", !empty($metaSettings["nxs_meta_operator$postfix"])?$metaSettings["nxs_meta_operator$postfix"]:'', false, false, self::makeInputName("nxs_meta_operator$postfix", $nt, $ii),'hui' ); echo '</div>';
            ?></div>
            <div class="nxs_mediumXL_field_txn"><label class="field_title"> <?php _e( 'Custom Field Value', 'social-networks-auto-poster-facebook-twitter-g' ); ?>:</label><br/>
                <input name="<?php echo self::makeInputName("nxs_meta_value$postfix", $nt, $ii); ?>[]" id="nxs<?php echo $nt.$ii; ?>_meta_value<?php echo $postfix; ?>" style="font-weight: bold; color: #005800; border: 1px solid #ACACAC; width: 95%;" value="<?php echo !empty($metaSettings["nxs_meta_value$postfix"])?$metaSettings["nxs_meta_value$postfix"][0]:'';?>"/>
            </div>        
          </div>
          <div class="">
          </div>
    ...
    // ## Meta
        ...
        for( $i = 1; $i <= $count_compares; $i++ ) { $postfix = $i == 1 ? '' : '_'. $i; $rel = $i == 1 ? '' : 'nxs_meta_compare_'. $i;            
            ...
            self::print_select( (!empty( $current_post->ID))?$current_post->ID:0, $post_meta_keys, 'nxs_meta_key'.$ntN, !empty($metaSettings['nxs_meta_key'])?$metaSettings['nxs_meta_key']:'', true, false, self::makeInputName('nxs_meta_key', $nt, $ii), 'nxsSelItAjxAdd', 'nxs_meta_key' ); echo '</div>';
